...
import studierichtingen from "./studierichtingenData";
...
async function uploadStudierichtingen() {
	console.log("Starting upload of studierichtingen data...");

	try {
		const { error } = await supabase
			.from("studierichtingen")
			.insert(studierichtingen);
		...
		console.log(`Uploaded ${studierichtingen.length} records.`);
	} catch (e) {
		console.error("Unexpected error:", e);
	}
}

uploadStudierichtingen();
